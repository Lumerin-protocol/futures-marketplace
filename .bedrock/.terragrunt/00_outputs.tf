...
output "github_actions_role_arn" {
  ...
  value       = (var.create_core) ? aws_iam_role.github_actions_futures[0].arn : null
}
output "github_actions_role_name" {
  description = "Name of the IAM role for GitHub Actions"
  value       = var.create_core ? aws_iam_role.github_actions_futures[0].name : null
}
output "marketplace_s3_bucket" {
  description = "S3 bucket name for marketplace deployment"
  value       = var.create_core ? aws_s3_bucket.marketplace[0].id : null
}
output "marketplace_cloudfront_distribution_id" {
  description = "CloudFront distribution ID for marketplace"
  value       = var.create_core ? aws_cloudfront_distribution.marketplace[0].id : null
}
