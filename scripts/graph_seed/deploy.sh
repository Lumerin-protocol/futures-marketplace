#!/bin/bash
#
# Local Graph Seed Script for Futures Marketplace Subgraph
# 
...
echo ""
echo -e "${BLUE}===============================================${NC}"
echo -e "${BLUE}🚀 Graph Seed Script - Futures Marketplace${NC}"
echo -e "${BLUE}===============================================${NC}"
...
case $ENV in
    dev)
        ...
        NETWORK="$DEV_NETWORK"
        FUTURES_ADDRESS="$DEV_FUTURES_ADDRESS"
        START_BLOCK_FUTURES="$DEV_START_BLOCK_FUTURES"
        ;;
    stg)
        DEPLOY_KEY="$STG_GRAPH_DEPLOY_KEY"
        SUBGRAPH_NAME="$STG_SUBGRAPH_NAME"
        NETWORK="$STG_NETWORK"
        FUTURES_ADDRESS="$STG_FUTURES_ADDRESS"
        START_BLOCK_FUTURES="$STG_START_BLOCK_FUTURES"
        ;;
    lmn)
        DEPLOY_KEY="$LMN_GRAPH_DEPLOY_KEY"
        SUBGRAPH_NAME="$LMN_SUBGRAPH_NAME"
        NETWORK="$LMN_NETWORK"
        FUTURES_ADDRESS="$LMN_FUTURES_ADDRESS"
        START_BLOCK_FUTURES="$LMN_START_BLOCK_FUTURES"
        ;;
    *)
...
IPFS_UPLOAD_URL="${IPFS_URL:-http://localhost:5001}"
IPFS_HASH=""
...
# Wait for content to propagate to global network
echo ""
