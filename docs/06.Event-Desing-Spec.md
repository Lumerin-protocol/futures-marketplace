...
Trigger: `liquidatePosition` (single lot) or `liquidatePositions` (batched
close-to-IM subset) — both permissionless. Per closed lot,
`_forceLiquidatePosition` first runs `_createOrMatchSingleOrder` for the
counterparty (may produce path-4/5/6 sub-events), then cash-settles at the
hashprice oracle price. A batch emits the block below once per lot in the
supplied set:
