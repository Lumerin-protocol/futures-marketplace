...
> `(user, expirationAt)` with `netQuantity` + `netEntryValue`. Orders carry signed whole-contract
...
```solidity
createOrder(uint256 price, uint256 expirationAt, int256 quantity)
cancelOrder(bytes32 orderId)
getOrder(bytes32) → OrderView { participant, price, quantity, expirationAt }
...
sequenceDiagram
  ...
  Taker->>Futures: createOrder(price, expirationAt, +qty)
