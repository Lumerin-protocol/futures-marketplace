...
This document is the single source of truth for `Futures.sol` **3.0.0** events, their
emission order per scenario, and the corresponding indexer entity operations.

> **Aggregate positions (contract `3.0.0`).** Positions are unilateral aggregates per
> `(user, deliveryAt)` with `netQuantity` + `netEntryValue`. Orders carry signed whole-contract
> quantity (one placement → one FIFO node; same-price placements do **not** merge). Matching,
> offset, liquidation, and settlement update each party's aggregate independently — there are
> no bilateral lots, no `destURL`, and no `Lot*` events.
...
```solidity
createOrder(uint256 price, uint256 deliveryAt, int256 quantity)
cancelOrder(bytes32 orderId)
getOrder(bytes32) → OrderView { participant, price, quantity, deliveryAt }
getUserOrders(address) → bytes32[]
getUserPosition(address, uint256 deliveryAt) → Position { netQuantity, netEntryValue }
getActiveDeliveryDates(address) → uint256[]

liquidateOrder(address user, bytes32 orderId)
liquidateOrders(address user)
liquidatePosition(address user, uint256 deliveryAt, uint256 closeQty)
liquidatePositions(address user, uint256[] deliveryAts, uint256[] closeQtys)

settlePosition(address user, uint256 deliveryAt)
settlePositions(address[] users, uint256[] deliveryAts)
recordSettlementPrice(uint256 deliveryAt)
```
...
```
OrderCreated(orderId, participant, price, quantity, deliveryAt)
```
...
sequenceDiagram
  ...
  Taker->>Futures: createOrder(price, deliveryAt, +qty)
...
Self-trade at the same price cancels own resting opposite orders (no book match).
...
Trigger: `liquidatePosition(user, deliveryAt, closeQty)` or batched
`liquidatePositions` (orders must already be cleared → else `OrdersStillOpen`).

```
PositionLiquidated(user, liquidator, deliveryAt, closedQuantity, pnl, fee=0)
```

Only **that user's** aggregate is reduced. Counterparties are unchanged.

Indexer: update `PositionSession` for `user` at `deliveryAt`; record PnL.

---

### Path 8 — Cash settlement at maturity

Trigger: permissionless `settlePosition(user, deliveryAt)` /
`settlePositions(users[], deliveryAts[])`.

First settle (or `recordSettlementPrice`) for an expiry pins the price:

```
SettlementPriceRecorded(deliveryAt, price, recordedBy)   // once per expiry
...
| ------------------- | ------------------------------------- | --------------------------------------------------------------------------- |
| `Order`             | `(tx, user, price, deliveryAt, side)` | Placement aggregate in a tx                                                 |
| `OrderEntry`        | `orderId`                             | One FIFO node; `remainingQuantity`                                          |
| `PositionSession`   | `(user, deliveryAt)`                  | Unilateral open-to-flat cycle; `netQuantity`, entry, realized PnL           |
| `Trade` / `Fill`    | per match                             | Built from `OrderMatched` (maker/taker legs)                                |
| `LiquidationTx`     | `txHash`                              | Dedupes liquidation counters                                                |
| `FuturesExpiration` | `deliveryAt`                          | Pinned `settlementPrice`, `recordedBy`                                      |
...
| `PositionLiquidated`      | Update session for `user`@`deliveryAt`; liquidation tx sentinel                  |
...
5. Cut over keeper, MM, and UI to the 3.0 ABI (`getUserOrders` / `getOrder`,
   `getActiveDeliveryDates` / `getUserPosition`, `settlePosition(user, deliveryAt)`,
   `liquidatePositions(user, deliveryAts[], closeQtys[])`).
