...
> **Contract `3.3.0`.** Orders carry signed whole-contract quantity. Positions are unilateral
> aggregates per `(user, expirationAt)`. Matching is a per-maturity limit order book (walk to
> limit, fill at maker price). `createOrder` / `createOrders` are GTC; `createOrderV2` /
> `createOrdersV2` add IOC / FOK. Each side's net updates independently — no bilateral lots.
...
### Time in force (`createOrderV2`)

| TIF | Behavior |
| --- | -------- |
| **GTC** (default `createOrder`) | Unfilled size rests on the book |
| **IOC** | Fill what is available now; cancel remainder (never rests) |
| **FOK** | Fill entire size now or revert `FillOrKillNotFilled` |
