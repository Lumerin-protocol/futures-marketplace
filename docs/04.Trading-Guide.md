...
> aggregates per `(user, deliveryAt)`. Matching updates each side's net independently — there
...
```
createOrder(price, deliveryAt, +qty)   // positive = buy / long
```
...
```
createOrder(price, deliveryAt, -qty)   // negative = sell / short
```
...
Hold until `deliveryAt`. Anyone may call `settlePosition(user, deliveryAt)` (typically a keeper).
