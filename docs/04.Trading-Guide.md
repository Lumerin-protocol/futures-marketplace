...
Hold the position until maturity (`deliveryAt`). Once matured, the position is cash-settled at the
oracle mark price via the permissionless `settlePosition` — anyone (typically a keeper) can trigger
it, and there is no payment, escrow, or destination endpoint to manage (see [Settlement](./05.Delivery-Settlement.md)).
