...
The protocol uses a per-maturity limit order book on a fixed price grid, collateralized margin accounts, and Chainlink-powered oracles for real-time hashprice discovery.
...
Create buy (long) or sell (short) orders via `createOrder(price, expirationAt, quantity, timeInForce)`:

- **Price**: Hashprice in settlement tokens
- **Maturity Date**: When the contract matures (`expirationAt`)
- **Quantity**: Signed whole contracts (`+` long / `−` short)

### 3. Order Matching

Incoming limit orders walk the opposite book for the same maturity from best price toward the taker's limit (price-time priority). Fills occur at each **maker's resting price** (price improvement allowed). Unfilled remainder rests at the taker's limit. Within each price level, matching is **FIFO**.
...
At maturity (`block.timestamp >= expirationAt`):
