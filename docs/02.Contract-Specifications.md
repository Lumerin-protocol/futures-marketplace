...
| ------------------------------------------------ | ------ | ------------ |
| `settlePosition(address user, uint256 deliveryAt)` | Active | Permissionless. Pins the expiry settlement price on first use, marks that user's aggregate to the pin, routes PnL via the insurance fund, clears the aggregate, emits `PositionSettled`. |
| `settlePositions(address[], uint256[])`          | Active | Batch of `(user, deliveryAt)` pairs; lengths must match. |
...
| `getActiveDeliveryDates(address)`                | Active | Expiries where the user still has a non-zero aggregate. |
...
| ----- | ------ | ------- |
| `PositionSettled(user, deliveryAt, closedQuantity, pnl, settlementPrice, settledBy)` | Active | User's aggregate cash-settled at the pinned price. |
| `SettlementPriceRecorded(deliveryAt, price, recordedBy)` | Active | Expiry settlement price pinned (once). |

### Errors

`settlePosition` reverts with `PositionNotExists()` (flat at that expiry) or
`PositionDeliveryNotStartedYet()` (before `deliveryAt`).
`recordSettlementPrice` reverts with `SettlementDateNotReached()` before `deliveryAt`.
